...
class InventoryModule(BaseInventoryPlugin, Constructable, Cacheable):
    ...
    def group_extractors(self):
        ...
        if self.api_version >= version.parse("2.11"):
            extractors["site_group"] = self.extract_site_groups
        if self.racks:
            extractors |= {
    ...
    def refresh_sites_lookup(self):
        ...
        self.sites_lookup_slug = {site["id"]: site["slug"] for site in sites}
        if self.site_data or self.prefixes:
            ...
            self.sites_lookup = {site["id"]: site for site in sites}
        else:
    ...
    def refresh_prefixes(self):
        ...
        url = f"{self.api_endpoint}/api/ipam/prefixes"
    ...
    def refresh_site_groups_lookup(self):
        ...
        url = f"{self.api_endpoint}/api/dcim/site-groups/?limit=0"
        site_groups = self.get_resource_list(api_url=url)
        self.site_groups_lookup = {
    ...
    def refresh_ipaddresses(self):
        ...
        url = f"{self.api_endpoint}/api/ipam/ip-addresses/?limit=0&assigned_to_interface=true"
        if self.fetch_all:
    ...
    def fetch_api_docs(self):
        try:
            status = self._fetch_information(f"{self.api_endpoint}/api/status")
